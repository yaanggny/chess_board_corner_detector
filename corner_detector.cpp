...
void CornerDetAC::createkernel(float angle1, float angle2, int kernelSize, Mat &kernelA, Mat &kernelB, Mat &kernelC, Mat &kernelD)
{
	...
	for (int u = 0; u < width; ++u){
		for (int v = 0; v < height; ++v){
			dtype vec[] = { u - kernelSize, v - kernelSize };//�൱�ڽ�����ԭ���ƶ���������
			dtype dis = std::sqrt(vec[0] * vec[0] + vec[1] * vec[1]);//�൱�ڼ��㵽���ĵľ���
			dtype side1 = vec[0] * (-sin(angle1)) + vec[1] * cos(angle1);//�൱�ڽ�����ԭ���ƶ���ĺ˽�����ת���Դ˲������ֺ�
...
void CornerDetAC::nonMaximumSuppression(Mat& inputCorners, vector<Point2f>& outputCorners, int patchSize, dtype threshold, int margin)
{
	...
	for (int i = margin + patchSize; i <= inputCorners.cols - (margin + patchSize+1); i = i + patchSize + 1)//�ƶ���鷽�飬ÿ���ƶ�һ������Ĵ�С
	{
		for (int j = margin + patchSize; j <= inputCorners.rows - (margin + patchSize+1); j = j + patchSize + 1)
		{
			...
			int flag = 0;
			for (int m = maxX - patchSize; m <= min(maxX + patchSize, inputCorners.cols - margin-1); m++)//���μ��
			{
...
void CornerDetAC::detectCorners(Mat &Src, Corners& mcorners,  dtype scoreThreshold, bool isrefine)
{
...
#endif	
		//compute mean
		imgCornerMean = (imgCornerA1 + imgCornerB1 + imgCornerC1 + imgCornerD1) / 4.0;//1.3 ���չ�ʽ���м���
	...
	nonMaximumSuppression(imgCorners, cornerPoints, 3, 0.025, 5);//1.5 �Ǽ���ֵ�����㷨���й��ˣ���ȡ���̸�ǵ�������
